...
test('keeps the administrator editor open and renders server validation errors', async () => {
  ...
  await user.type(screen.getByLabelText(/^name$/i), 'Admin'); await user.type(screen.getByLabelText(/email/i), 'admin@example.test'); await user.type(screen.getByLabelText(/^password$/i), 'long-enough-password')
  await user.click(screen.getByRole('button', { name: /save administrator/i }))
...
test('keeps the edit administrator sheet open when rename validation fails', async () => {
  const administrator = { id: '11111111-1111-4111-8111-111111111111', name: 'Existing admin', email: 'existing@example.test', role: 'ADMIN', active: true, createdAt: '2026-08-17T00:00:00.000Z', updatedAt: '2026-08-17T00:00:00.000Z' }
