...
export default function OriginalSets() {
  ...
  const designFor = (set: OriginalSetSummary) => designs.find((design) => design.id === set.designId)
  const designName = (set: OriginalSetSummary) => designFor(set)?.name ?? 'Unknown design'
  const recentIncomplete = visible.find((set) => !set.verifiedAt)
  const verifiedCount = visible.filter((set) => Boolean(set.verifiedAt)).length
  ...
  return <><PageHeader title="Original sets" actions={<><label className="sr-only" htmlFor="onboard-design">Active design</label><select id="onboard-design" value={designId} onChange={(event) => setDesignId(event.target.value)}><option value="">Choose active design</option>{designs.map((design) => <option value={design.id} key={design.id}>{design.designCode} — {design.name}</option>)}</select><button type="button" className="button" disabled={creating} onClick={() => void createNext()}>{creating ? 'Creating…' : 'Onboard next set'}</button></>} />
    <section className="original-sets-page"><p className="page-copy">Manage original sets. Your tailor groupings stay preserved as you progress.</p>{error ? <ErrorBanner message={error} /> : null}
      <div className="original-sets-metrics" aria-label="Original set summary"><span><b>{verifiedCount}</b> Verified</span><span><b>{visible.length - verifiedCount}</b> In progress</span>{recentIncomplete ? <button type="button" className="resume-summary" onClick={() => navigate(`/original-sets/${recentIncomplete.id}`)}>Resume {recentIncomplete.originalSetCode} · {recentIncomplete.onboarding?.stage ?? 'PHOTO'}</button> : null}</div>
      ...
      {visible.length === 0 ? <EmptyState title="No original sets" description="Onboard the next tailor set when an active design is ready." /> : <div className="data-table original-sets-table"><div className="table-header"><span>Design</span><span>Original set code</span><span>Items</span><span>Progress</span><span>Status</span><span>Action</span></div>{visible.map((set) => <div className="table-row" key={set.id}><span className="design-cell"><i className="design-colors" aria-hidden="true"><b style={{ background: designFor(set)?.primaryColor ?? '#ded8d4' }} /><b style={{ background: designFor(set)?.secondaryColor ?? '#fff' }} /></i>{designName(set)}</span><strong>{set.originalSetCode}</strong><span>{set.onboarding?.generatedExpectedItemCount ?? set.inventoryItemCount} of {set.onboarding?.expectedItemCount ?? set.inventoryItemCount}</span><span className="set-progress"><b>{progress(set)}%</b><i><em style={{ width: `${progress(set)}%` }} /></i><small>{set.onboarding?.stage ?? 'PHOTO'}</small></span><span className={set.verifiedAt ? 'verified-badge' : 'incomplete-badge'}>{set.verifiedAt ? 'Verified' : 'Incomplete'}</span><button className={set.verifiedAt ? 'button button-secondary' : 'button'} type="button" aria-label={`${set.verifiedAt ? 'View' : 'Resume'} ${set.originalSetCode}`} onClick={() => navigate(`/original-sets/${set.id}`)}>{set.verifiedAt ? 'View' : 'Resume'}</button></div>)}</div>}</section></>
}
